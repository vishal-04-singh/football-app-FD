...
export const TournamentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getMatchStats = useCallback(async (matchId: string): Promise<MatchStats | null> => {
    try {
  ...
  // Complete a match
  const completeMatch = useCallback(async (matchId: string, finalStats?: MatchStats) => {
    try {
